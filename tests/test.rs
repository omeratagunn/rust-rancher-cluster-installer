...
fn test_k3s_master_concat() {
    let res = rancherinstaller::build_k3s_master_command(&"v1.25.4+k3s1".to_string());
...
fn test_k3s_node_concat() {
    let res = rancherinstaller::build_k3s_node_command(
...
fn test_strip_trailing() {
    ...
    rancherinstaller::strip_trailing_nl(&mut test_str);
